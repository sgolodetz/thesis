...
\begin{stusubfig}{p}
	...
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-unsmoothed.png}\hspace{4mm}}%
	%
	\hspace{8mm}%
	%
	\subfigure[Smoothed image]
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-smoothed.png}\hspace{4mm}}%
	%
	\\
	%
	\subfigure[Gradient magnitude of the unsmoothed image (after equalising histogram)]
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-unsmoothedgm-eh.png}\hspace{4mm}}%
	%
	\hspace{8mm}%
	%
	\subfigure[Gradient magnitude of the smoothed image (after equalising histogram)]
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-smoothedgm-eh.png}\hspace{4mm}}%
	%
	\\
	%
	\subfigure[Watershed of the unsmoothed image]
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-unsmoothedws.png}\hspace{4mm}}%
	%
	\hspace{8mm}%
	%
	\subfigure[Watershed of the smoothed image]
	{\hspace{4mm}\includegraphics[height=5.8cm]{segmentation/segmentation-watershed-adfexample-smoothedws.png}\hspace{4mm}}%
\caption{The results of running a watershed algorithm on an image smoothed by anisotropic diffusion filtering (ADF) are far superior, because ADF smoothes away all the unnecessary detail within features whilst preserving the crucial edges between them. The smoothed results here were produced using $20$ iterations of the gradient anisotropic diffusion method from ITK, with a conductance of $1.0$ and a time step of $0.0625$.}
...
\caption[Windowing images before segmenting them leads to a less pronounced oversegmentation]{Windowing images before segmenting them leads to a less pronounced oversegmentation -- note in particular the generally larger regions in (b)}
...
\caption[The propagation step of Marcotegui's waterfall algorithm, illustrated on the graph in \cite{marcotegui05}]{The propagation step of Marcotegui's waterfall algorithm, illustrated on the graph in \cite{marcotegui05}: blue edges are those that have already been elided, green edges are the ones currently under consideration, and red edges are those that will not be elided.}
...
%---
\stufigexx{height=20cm}{segmentation/segmentation-ipfconstruction-goal.png}{The goal of the IPF construction process}{The goal of the IPF construction process: coloured edges represent tree (forest) links between layers, whilst the dotted edges in each layer represent the layer's region adjacency graph}{fig:segmentation-ipfconstruction-goal}{p}
%---
...
In this chapter, we saw how to generate IPFs from medical images using the morphological watershed and waterfall transforms, thereby accounting for the third and fourth contributions claimed in Chapter~\ref{chap:introduction}:
%
\begin{enumerate}
\setcounter{enumi}{2}
...
\end{enumerate}
%
The next chapter will show how the generated IPFs can be used when automatically identifying salient features in the original images.