...
Once the MST has been constructed, each recursive pass of Nicholls' algorithm is $O(Vd)$, where $d$ is the maximum degree of a vertex (i.e.~the maximum number of edges adjacent to any vertex in the MST). To see this, consider that the algorithm visits each vertex in the MST precisely once (the factor of $V$), and that for each vertex it finds a minimally-valued child edge ($O(d)$), does some constant-time comparison and remarking work and then elides all the non-guard child edges (also $O(d)$).
...
Medium: Whilst not being notably difficult to code (particularly in a language with good standard libraries), Marcotegui's algorithm is still somewhat trickier to implement than the other two algorithms. Determining the local minima explicitly takes some work, and the propagation step uses a priority queue.

\subparagraph{Nicholls' Algorithm}

Very Easy: Once the MST has been constructed, Nicholls' algorithm is a one-pass, recursive, tree-based algorithm, and can be implemented in a very small amount of code.

\subparagraph{My Algorithm}

Easy: Once the MST has been constructed, my algorithm is a two-pass, recursive, tree-based algorithm and can be implemented very simply.
