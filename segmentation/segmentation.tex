...
Once the MST has been constructed, each iteration of Nicholls' algorithm is $\Theta(V)$. To justify this, consider that there are by definition $V - 1$ edges in the MST, and that the algorithm accesses each edge at least once (hence $\Omega(V)$) and at most a constant number of times (hence $O(V)$). More specifically, each edge is processed itself, accessed again in the course of finding the lowest child of its parent, and then possibly accessed once more if it is to be elided.

The overall complexity of Nicholls' algorithm is the same as the complexity of the minimum spanning tree construction process (whichever MST algorithm is used), since the complexity of a constant number of iterations is still $\Theta(V)$, and all the MST algorithms are $\omega(V)$.
...
Whilst not being notably difficult to code (particularly in a language with good standard libraries), Marcotegui's algorithm is still somewhat trickier to implement than the other two algorithms. Determining the local minima explicitly takes some work, and the propagation step requires a priority queue.

\subparagraph{Nicholls' Algorithm}

Once the MST has been constructed, Nicholls' algorithm is a one-pass, recursive, tree-based algorithm, and can be implemented very simply in a small amount of code.

\subparagraph{My Algorithm}

Once the MST has been constructed, my algorithm is a two-pass, recursive, tree-based algorithm and can be implemented very simply.
