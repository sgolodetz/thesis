...
It is possible to perform a case analysis for the first pass and the route resolution step 2(a) as well (see Figures~\ref{fig:segmentation-waterfall-smg-pass1cases} and \ref{fig:segmentation-waterfall-smg-resolutioncases}), and this is helpful to clarify how the algorithm works. As illustrated in the diagram, there are six cases for the first pass: (i) the node has a unique path of steepest descent down a child edge; (ii) the node has a unique path of steepest descent up the parent edge; (iii) the node has more than one path of steepest descent (the parent edge is not a possible path of steepest descent); (iv) the node has more than one \emph{downwards} path of steepest descent (but the parent edge still needs to be considered); (v) there is no flow out of the node and (vi) there is no downwards flow out of the node (but the parent edge still needs to be considered). For the resolution step, there are four cases: (i) the arrow from the parent node points towards us; (ii) there is no flow from the parent node; (iii) there is an equally good route via the parent node and (iv) there is a better route via the parent node.

% TODO: fig:segmentation-waterfall-smg-pass1cases
% TODO: fig:segmentation-waterfall-smg-resolutioncases
