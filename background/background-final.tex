...
%(Clusterings are evaluated in \cite{arthur07} in terms of a value $\phi$, `the total squared distance between each point and its closest center', where an optimal clustering has value $\phi_{\mbox{OPT}}$ that minimises this quantity. A clustering is $O(\log k)$-competitive if its $\phi$ satisfies $\phi / \phi_{\mbox{OPT}} \in O(\log k)$.)
