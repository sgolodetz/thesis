...
\index{segmentation!thresholding|(}
...
\caption[Segmenting the brighter of two adjacent objects in an 8-bit greyscale image using binary thresholding]{Segmenting the brighter of two adjacent objects in an 8-bit greyscale image using binary thresholding (the objects are easily separated in this case because the distribution of grey values in the image is bimodal)}
...
\caption[Thresholding failure due to overlapping grey value distributions]{Trying to segment the kidneys in the CT image in (a) using thresholding alone doesn't work. If we choose a wide foreground range, as in (b), we oversegment the kidneys, dragging in lots of unrelated features across the image. If we choose a narrower range to try and prevent this, as in (c), we miss pieces of the kidneys themselves and end up with holes. Note that even in (c), we are unable to prevent other features being segmented as well as the kidneys, as the grey value distributions of the kidneys and other features overlap.}
...
%---
\stufigexx{height=8cm}{background/background-segmentation-regiongrowing-lin.png}{The candidate regions searched for kidney seeds in \cite{lin06}}{The candidate regions searched for kidney seeds in \cite{lin06} \stuhl{(\copyright{} 2006 IEEE, reproduced from the paper in question with kind permission)}}{fig:background-segmentation-regiongrowing-lin}{p}
%---
...
\index{watershed transform!background|(}
...
%---
\stufigexx{height=9cm}{background/background-segmentation-snakes-gvfexample.png}{The basic snakes method suffers from a poor capture range, but techniques such as gradient vector flow \cite{xu98} can mitigate this}{The basic snakes method suffers from a poor capture range, but techniques such as gradient vector flow \cite{xu98} can mitigate this -- the figure \stuhl{(\copyright{} 1998 IEEE, reproduced from the paper in question with kind permission)} shows a GVF snake deforming from an initial, circular contour to segment the left ventricle of a human heart.}{fig:background-segmentation-snakes-gvfexample}{p}
%---
...
\caption[The results of using the \emph{geodesic active contours} level set method from the \emph{Insight Toolkit} to identify the kidneys in an example CT image]{The results of using the \emph{geodesic active contours} level set method from the \emph{Insight Toolkit} to identify the kidneys in an example CT image. The parameters had to be carefully tweaked by hand for each kidney, which is a downside of level set approaches.}
\label{fig:background-segmentation-levelsets-ct}
\end{stusubfig}
%---

%---
\stufigexx{height=5.5cm}{background/background-segmentation-atlas-park.png}{Volume rendering of a probabilistic atlas constructed by Park et al.\ in \cite{park03}}{Volume rendering of a probabilistic atlas constructed by Park et al.\ in \cite{park03} \stuhl{(\copyright{} 2003 IEEE, extracted from Figure $2$ of the paper in question with kind permission)} -- the colours and brightnesses of the voxels depend on the probability values for the various organs at the voxels in question}{fig:background-segmentation-atlas-park}{p}
%---
...
\index{segmentation!neural network-based|(}
...
%---
\stufigexx{height=20cm}{background/background-segmentation-neuralnets-tsai.png}{Schematic diagram of the neural-network-based boundary detection method used in \cite{tsai94}}{Schematic diagram of the neural-network-based boundary detection method used in \cite{tsai94} \stuhl{(\copyright{} 1994 IEEE, reproduced from the paper in question with kind permission)}}{fig:background-segmentation-neuralnets-tsai}{p}
%---
...
%---
\stufigexx{height=8cm}{background/background-segmentation-neuralnets-lee.png}{The architecture of the contextual neural network used in \cite{lee03}}{The architecture of the contextual neural network used in \cite{lee03} \stuhl{(\copyright{} 2003 IEEE, reproduced from the paper in question with kind permission)}}{fig:background-segmentation-neuralnets-lee}{p}
%---

%---
\stufigexx{height=8cm}{background/background-segmentation-neuralnets-leeresults.png}{Results of the neural network method described in \cite{lee03}}{Results of the neural network method described in \cite{lee03} \stuhl{(\copyright{} 2003 IEEE, reproduced from the paper in question with kind permission)}, showing the identified spine, left kidney, spleen and liver}{fig:background-segmentation-neuralnets-leeresults}{p}
%---
...
\index{segmentation!fuzzy C-means|(}
...
\index{segmentation!normalized cuts|(}
...
%---
\stufigexx{height=8cm}{background/background-segmentation-hybrid-soler.png}{An automatic 3D reconstruction of patients from medical images, as performed by the IRCAD surgical planning tools developed using the work of Soler et al.}{An automatic 3D reconstruction of patients from medical images, as performed by the IRCAD surgical planning tools developed using the work of Soler et al. \stuhl{The figure is reproduced from \cite{soler-imvie}, with the kind permission of Luc Soler.}}{fig:background-segmentation-hybrid-soler}{t}
%---
...
\index{partition forests!background|(}
...
%---
\stufigex{height=7cm}{background/background-partitionhierarchies-treeexample.png}{A partition tree for the set $\{a,b,c,d,e\}$ -- note that the sets represented by child nodes partition the set represented by their parent, e.g.~$\{c\}$ and $\{d,e\}$ partition $\{c,d,e\}$.}{fig:background-partitionhierarchies-treeexample}{p}
%---
...
%---
\stufigex{height=7cm}{background/background-partitionhierarchies-completetree.png}{A \emph{complete} partition tree corresponding to the incomplete one in Figure~\ref{fig:background-partitionhierarchies-treeexample}}{fig:background-partitionhierarchies-completetree}{p}
%---
...
Hierarchical pathfinding is an approach designed to reduce these prohibitive storage costs. The essence of the idea is to partition the graph into clusters. This partitioning of the graph into clusters creates a two-layer partition hierarchy (in this case a partition forest) of the graph nodes. Pathfinding schemes can then be devised that still produce optimal paths, but only store the substantially smaller transition tables for paths within each cluster and a transition table for an inter-cluster `super-graph' -- \stuhl{for example the scheme employed in \cite{kim98} (see Figure~\ref{fig:background-partitionhierarchies-pathfinding-kim})}. The kind of storage savings that can be expected from this sort of approach are quantified by Dickheiser in \cite{dickheiser04}.

%---
\begin{stusubfig}{p}
	\subfigure[Flat graph]{\includegraphics[height=4cm]{background/background-partitionhierarchies-pathfinding-kimredrawn-a.png}}%
	\hspace{4mm}%
	\subfigure[Partition]{\includegraphics[height=4cm]{background/background-partitionhierarchies-pathfinding-kimredrawn-b.png}}%
	\\
	\subfigure[Super-graph]{\includegraphics[height=4cm]{background/background-partitionhierarchies-pathfinding-kimredrawn-c.png}}%
	\hspace{4mm}%
	\subfigure[Search space]{\includegraphics[height=4cm]{background/background-partitionhierarchies-pathfinding-kimredrawn-d.png}}%
\caption[An illustration of the hierarchical pathfinding scheme employed in \cite{kim98}: partitioning a large graph into clusters allows us to only store transition tables for each cluster and the super-graph, rather than a transition table for the entire graph]{\stuhl{An illustration of the hierarchical pathfinding scheme employed in \cite{kim98}: partitioning a large graph (a) into clusters allows us to only store transition tables for each cluster (b) and the super-graph (c), rather than a transition table for the entire graph. When finding paths, all the necessary information is contained in the transition tables for the source cluster, the destination cluster and the super-graph (d). (The figure is a redrawn version of Figure $2$ from Kim et al.\ \cite{kim98}.)}}
\label{fig:background-partitionhierarchies-pathfinding-kim}
\end{stusubfig}
%---
