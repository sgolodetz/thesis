...
\item The nodes must share a common parent in the forest. Note that this implies that they must all be in the same layer. (Assuming the $k$ nodes share a common layer $m$, then the cost of looking up the parent for each node is $O(\log n_m)$, whence the total cost of this check is $O(k \log n_m)$.)
\item The union of the objects represented by the nodes to be merged must be connected. (This can be checked in $O(k \log e_m)$ time. To see this, examine the \texttt{are_connected} and \texttt{find_connected_component} listings in Appendix~\ref{chap:appendixpf}. The worst case is when all the nodes must be visited, i.e.~when there is a single connected component. When that happens, over the course of the process, every node is erased from the set of input nodes and inserted into the set of result nodes -- $O(\log (k!))$ overall. Each node is also pushed onto the queue and popped off it again -- $O(k)$ overall. Every node but the seed is also looked up during the \texttt{contains} call -- $O(\log (k!))$ overall, because the size of the input node set decreases after each such call. The dominant cost of $O(k \log e_m)$ comes from having to look up the adjacent edges for each input node in the tree-based data structure storing the edges for forest layer $m$.)

\end{enumerate}

\noindent The overall cost of checking the preconditions is thus $O(k \log e_m)$, since $n_m \in O(e_m)$ (the adjacency graphs we're dealing with are connected).
...
Although I will not show the long-winded derivation here, the complexity of the algorithm as a whole can be simplified down to $O(\#children(\mathit{canonical}) + (\sum_{x \in others} \#\mathit{children}(x)) \log n_b + (\sum_{x \in \mathit{others}} \#\mathit{adjacent\_edges}(x)) \log e_m)$. Note in passing that this is upper-bounded by $O(n_b \log n_b + e_m \log e_m)$.
...
\begin{stusubfig}{p}
	...
	\\
	\subfigure[Resulting layer $1$ graph]{\includegraphics[width=.22\linewidth]{ipfs/ipfs-forest-unzipping-b-graph1.png}}%
	\hspace{8mm}%
	\subfigure[Resulting layer $2$ graph]{\includegraphics[width=.22\linewidth]{ipfs/ipfs-forest-unzipping-c-graph2.png}}%
	\hspace{8mm}%
	\subfigure[Resulting layer $3$ graph]{\includegraphics[width=.22\linewidth]{ipfs/ipfs-forest-unzipping-d-graph3.png}}%
...
%---
\begin{stusubfig}{p}
	\subfigure[Layer 1 before unzipping]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-a.png}}%
	\hspace{8mm}%
	\subfigure[Layer 2 before unzipping]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-b.png}}%
	\\
	\subfigure[Layer 3 before unzipping]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-c.png}}%
	\hspace{8mm}%
	\subfigure[Select a node to unzip]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-d.png}}%
	\\
	\subfigure[Click the menu item]{\includegraphics[width=.35\linewidth]{ipfs/ipfs-forest-unzipping-gui-e.png}}%
	\hspace{8mm}%
	\subfigure[Enter the target layer]{\includegraphics[width=.35\linewidth]{ipfs/ipfs-forest-unzipping-gui-f.png}}%
	\\
	\subfigure[Layer 2 after unzipping]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-g.png}}%
	\hspace{8mm}%
	\subfigure[Layer 3 after unzipping]{\includegraphics[width=.25\linewidth]{ipfs/ipfs-forest-unzipping-gui-h.png}}%
...
\begin{stusubfig}{p}
	...
	\\
	\subfigure[Initial layer $1$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-a-graph1.png}}%
	\hspace{8mm}%
	\subfigure[Initial layer $2$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-a-graph2.png}}%
	\hspace{8mm}%
	\subfigure[Initial layer $3$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-a-graph3.png}}%
	\\
	\subfigure[Resulting layer $1$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-d-graph1.png}}%
	\hspace{8mm}%
	\subfigure[Resulting layer $2$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-c-graph2.png}}%
	\hspace{8mm}%
	\subfigure[Resulting layer $3$ graph]{\includegraphics[width=.24\linewidth,height=.26\linewidth]{ipfs/ipfs-forest-zipping-b-graph3.png}}%
...
\textbf{Algorithm} & \textbf{Preconditions} & \textbf{Execution} & \textbf{Undo} \\
\hline\hline
Layer Cloning & $O(1)$ & $O(e_b)$ / $O(\log (e_b!))$ & $O(n_b \log n_d)$ \\
\hline
Layer Deletion & $O(1)$ & $O(n_b \log n_d)$ & $O(n_b \log n_b)$ \\
\hline
Sibling Node Merging & $O(k \log e_m)$ & \begin{tabular}{c} $O\left[ \#\mathit{children}(\mathit{canonical}) \; + \right.$ \\ $\left( \sum_{x \in \mathit{others}} \#\mathit{children}(x) \right) \log n_b \; +$ \\ $\left. \left( \sum_{x \in \mathit{others}} \#\mathit{adjacent\_edges}(x) \right) \log e_m \right]$ \end{tabular} & ? \\
\hline
Node Splitting & ? & ? & ? \\ 
\hline
Unzipping & ? & ? & ? \\
...
Zipping & ? & ? & ? \\
\hline
Non-Sibling Node Merging & $O(k)$ & ? & ? \\
\hline
Parent Switching & ? & ? & ? \\
\hline\hline
Node Selection & N/A & ? & N/A \\
\hline
Node Deselection & N/A & ? & N/A \\
\hline
View At Layer & N/A & ? & N/A \\
\hline
Undo Modification & N/A & ? & N/A \\
\hline
Redo Modification & N/A & ? & N/A \\
\hline
Layer Was Cloned & N/A & ? & N/A \\
\hline
Layer Will Be Deleted & N/A & ? & N/A \\
\hline
Layer Was Undeleted & N/A & ? & N/A \\
\hline
Nodes Will Be Merged & N/A & ? & N/A \\
\hline
Nodes Were Merged & N/A & ? & N/A \\
\hline
Node Was Split & N/A & ? & N/A
\end{tabular}
\end{center}
\caption{This table summarises the complexities of the various algorithms described in this chapter. To understand the notation used, readers are encouraged to refer back to the sections describing the algorithms concerned.}
\end{table}
%\end{landscape}
