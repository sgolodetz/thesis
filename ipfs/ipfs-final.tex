...
\end{eqnarray*}
%
Simplifying even further, this can be upper-bounded by $O(n_b \log n_b + e_m \log e_m)$.
...
\]
%
Since $\#\mathit{children}(node) \le n_b$, the first part of the sum is $O(n_b \log e_b)$. The second part is $O(e_b (\log n_b + \log e_s))$, since each edge in the layer below can only appear as the adjacent edge of at most two children. A simpler upper-bound for the entire algorithm is thus just $O(e_b (\log n_b + \log e_s))$.
