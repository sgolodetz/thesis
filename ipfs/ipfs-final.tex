...
\hline\hline
Layer Cloning & $O(1)$ & Amortised \{$O(e_b)$ / $O(\log (e_b!))\}$ & $O(n_b \log n_d)$ \\
\hline
Layer Deletion & $O(1)$ & $O(n_b \log n_d)$ & Amortised $O(n_b \log n_b)$ \\
\hline
...
Unzipping & ? & ? & ? \\
\hline
Zipping & ? & $O(\sum_{m=\mathit{lowLayer}}^{\mathit{highLayer}} \mathit{sibling\_merge\_cost}(\mathit{nodes}_m))$ & ? \\
\hline
...
\end{tabular}
\end{center}
\caption{A summary of the complexities of the more important algorithms described in this chapter. To understand the notation used, readers are encouraged to refer back to the sections describing the algorithms concerned.}
\end{table}
%\end{landscape}
