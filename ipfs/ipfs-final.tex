...
\hline
Sibling Node Merging & $O(\#\mathit{nodes} \log e_m)$ & \begin{tabular}{c} $O\left[ \#\mathit{children}(\mathit{canonical}) \; + \right.$ \\ $\left( \sum_{x \in \mathit{others}} \#\mathit{children}(x) \right) \log n_b \; +$ \\ $\left. \left( \sum_{x \in \mathit{others}} \#\mathit{adjacent\_edges}(x) \right) \log e_m \right]$ \end{tabular} & ? \\
\hline
Node Splitting & ? & ? & ? \\ 
\hline
Unzipping & ? & ? & ? \\
...
Zipping & ? & ? & ? \\
\hline
Non-Sibling Node Merging & $O(\#\mathit{nodes})$ & ? & ? \\
\hline
Parent Switching & ? & ? & ? \\
\hline\hline
