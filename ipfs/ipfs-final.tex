...
%In terms of complexity, we can neglect the cost of actually erasing the layer from the forest (with a na\"ive implementation, this can be linear in the number of layers in the forest, but that number is rarely greater than $10$ for the domains in which partition forests see use). The dominant cost of deleting a layer is in updating the forest links. We use the same notation for node and edge counts as in the layer cloning analysis. The first loop in the pseudo-code (which clears the child sets in layer $A$) is $\Theta(n_a)$, i.e.~linear in the number of nodes in layer $A$. TODO
