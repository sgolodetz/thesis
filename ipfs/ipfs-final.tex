...
%---
\stufigex{height=22cm}{ipfs/ipfs-concept.png}{The concept of a partition forest (see main text for discussion)}{fig:ipfs-concept}{p}
%---
...
\begin{lstlisting}[style=Prototype]
Modification select_node(const PFNodeID& node);
\end{lstlisting}
...
\begin{lstlisting}[style=Prototype]
Modification deselect_node(const PFNodeID& node);
\end{lstlisting}
...
As previously mentioned, the \emph{node selection} and \emph{node deselection} algorithms return selection modifications that can be used to undo their effects. The algorithm reinserts nodes that were erased from the representation, and removes nodes that were inserted:
...
Selection modifications that were undone by \texttt{undo_modification} can also be redone (there is no need to execute the original command a second time, which would involve recalculating the necessary nodes to erase/insert). The algorithms re-erases any nodes in the \texttt{erased} set, and re-inserts any nodes in the \texttt{inserted} set:
