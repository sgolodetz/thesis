...
\begin{stusubfig}{p}
	...
	{\hspace{8mm}\includegraphics[height=6cm]{featureid/featureid-3d-spinalcanalidentification-transparent-BT-2-60-80.png}\hspace{8mm}}%
	%
	\hspace{4mm}%
	%
	\subfigure[Series BT-2 (Slices 60--80), Spinal Canal Only]
	{\hspace{12mm}\includegraphics[height=6cm]{featureid/featureid-3d-spinalcanalidentification-solid-BT-2-60-80.png}\hspace{12mm}}%
	%
	\\
	%
	\subfigure[Series SD-2 (Slices 70--90), Spine and Spinal Canal]
	{\hspace{16mm}\includegraphics[height=6cm]{featureid/featureid-3d-spinalcanalidentification-transparent-SD-2-70-90.png}\hspace{16mm}}%
	%
	\hspace{4mm}%
	%
	...
	{\hspace{12mm}\includegraphics[height=6cm]{featureid/featureid-3d-spinalcanalidentification-transparent-EJ-2-20-40.png}\hspace{12mm}}%
	%
	\hspace{4mm}%
	%
...
%---
\stufigex{height=9cm}{featureid/featureid-3d-ribsidentification-region.png}{The spatial constraints for rib seeds -- they must be at least partly in one of the regions labelled `R', and no part of them must lie in the region labelled `T'}{fig:featureid-3d-ribsidentification-region}{h}
%---
...
%---
\stufigex{height=9cm}{featureid/featureid-3d-aortaidentification-region-overlay.png}{TODO -- The spatial constraints for aorta seeds -- they (a) must lie at least partly in the small, green box just above the spine, (b) may lie partly in the boxes marked in yellow, and (c) must not lie below or to the left of the red lines}{fig:featureid-3d-aortaidentification-region}{h}
%---
...
\end{itemize}
%
As illustrated in Figure~\ref{fig:featureid-3d-kidneysidentification-results}, the identifier has been used to produce appealing results for volumes from a number of different patients. However, as with some of the other identifiers, it needs further work to make it more robust on arbitrary series. In particular, the current approach struggles to deal with kidneys containing multiple cysts (see Figure~\ref{?}), because they tend to prevent a kidney being represented by a single region in the partition forest. For the same reason, it also has problems with kidneys affected by tumours. In the EB-2 case shown in Figure~\ref{fig:featureid-3d-kidneysidentification-results}(d), this results in the left kidney being completely missed due to the presence of a large tumour (see Figure~\ref{?}). It is anticipated that it should be possible to solve such problems using stratified region growing in the future.
...
Despite being relatively easily distinguished by a human looking at the CT images, the spleen does not in general tend to be perfectly represented by a single region in the partition forest. This is primarily due to rather weak boundaries between the spleen and some of its surrounding features, meaning that they tend to be inadvertently merged together during the segmentation process. Higher regions in the partition forest tend to oversegment the spleen, whilst lower ones tend to undersegment it (see Figure~\ref{?}).
...
%
\begin{itemize}
\item Be in the lowest branch layer (as noted, regions in higher layers tend to oversegment the spleen).
\item Have a mean grey value in the range $[140,170]$.
\item Stretch sufficiently far to the right of the image (at least part of them must be in the right-hand $20\%$ of the image).
