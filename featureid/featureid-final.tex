...
%---
\stufigexx{height=8cm}{featureid/featureid-3d-ribsidentification-region.png}{The spatial constraints for rib seeds}{The spatial constraints for rib seeds -- they must be at least partly in one of the regions labelled `R', and no part of them must lie in the region labelled `T'}{fig:featureid-3d-ribsidentification-region}{h}
%---
...
\item \stuhl{Have a voxel count of less than $500$ voxels per (axial) slice.}
\item Have a mean grey value $\ge 200$.
...
%
\begin{itemize}
\item Have a mean grey value in the range $[150,180]$ \stuhl{(the aorta is generally a relatively bright grey in colour)}.
\item Have a voxel count of between $350$ and $750$ voxels per (axial) slice \stuhl{(determined empirically by examining aorta cross-sections in a number of image series)}.
...
\item Have a mean grey value in the range $[150,190]$ \stuhl{(the liver is generally a relatively bright grey in colour)}.
\item Be at least partly on the left-hand side of the image.
\end{itemize}
%
\stuhl{As with previous identifiers, the specific limit values were chosen empirically via careful examination of the relevant regions from a number of different image series.} Put simplistically, the filtering process in this case essentially entails trying to find large, grey regions that are on the left-side of the image (i.e.~the right side of the body) and that do not oversegment the liver. Having found a suitable set of liver candidates, the identifier next picks the one which is furthest to the left of the image as the `best' candidate. It grows a region from this using stratified region growing, using a grow condition that only causes adjacent regions to be added if they:
%
\begin{itemize}
...
\begin{itemize}
\item In the range $[150,190]$ \stuhl{(the same as the range for liver seeds)}
...
\end{itemize}
%
\stuhl{The values for the latter two criteria were determined experimentally.} As is usual where region growing approaches are concerned, the region grown using this condition may well contain holes. The final step of the identifier is therefore to fill these in using conditional morphological closing. The condition applied is that regions should only be dilated to / eroded from where their mean grey value is $\ge 140$ \stuhl{(i.e.~slightly lower than the low end of the chosen liver seed range)} -- this allows regions that are very slightly darker than would be acceptable for a liver seed to be filled in, whilst trying to prevent the outer boundary of the identified liver from being damaged in the process.
...
\begin{stusubfig}{!b}
	...
	{\includegraphics[height=7cm]{featureid/featureid-3d-liveridentification-SD-2-55-87-c.png}}%
	%
	\\
	%
	\subfigure[Series EB-2 (Slices 40--78)]
	{\includegraphics[height=7cm]{featureid/featureid-3d-liveridentification-EB-2-40-78-b.png}}%
...
\item \stuhl{Have an $x$-$y$ \emph{aspect ratio} of between $\frac{1}{4}$ and $4$} -- i.e.~neither the $x$ nor $y$ dimension of the region's bounding box must be greater than $4$ times the other \stuhl{(kidneys are relatively compact)}.
\item Have a voxel count of between $2000$ and $6000$ voxels per (axial) slice \stuhl{(determined empirically by examining kidney cross-sections in a number of image series)}.
\end{itemize}
%
...
\begin{stusubfig}{!t}
	...
	{\includegraphics[height=7cm]{featureid/featureid-3d-kidneysidentification-tumourhandling-a.png}}%
	%
	\hspace{4mm}%
	%
	\subfigure[The relevant partition in the forest]
	{\hspace{4mm}\includegraphics[height=7cm]{featureid/featureid-3d-kidneysidentification-tumourhandling-b.png}\hspace{4mm}}%
...
\item Have a mean grey value in the range $[140,170]$ \stuhl{(the spleen is generally a relatively bright grey in colour)}.
\item Stretch into the right-hand $20\%$ of the image (see Figure~\ref{fig:featureid-3d-spleenidentification-region}). \stuhl{This is a weak constraint used to specify that the spleen lies on the left-hand side of the body. The $20\%$ is a conservative value, determined empirically by examining slices from a number of series.}
\item Have a voxel count of between $1000$ and $7000$ voxels per (axial) slice. \stuhl{The limits are loose bounds determined by examining spleen cross-sections in slices from a number of series.}
\end{itemize}
%
