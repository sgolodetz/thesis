...
\begin{lstlisting}[style=Default]
function combine : (lhs : Selection; rhs : Selection) $\to$ $\emptyset$
...
\begin{lstlisting}[style=Default]
function combine_using_leaves : (lhs : Selection; rhs : Selection) $\to$ $\emptyset$
	// Note: This method should only be invoked on newly-created selections.
	assert empty();
...
\begin{lstlisting}[style=Default]
function replace_with_selection_impl : (selection : Selection) $\to$ Modification
	var modification : Modification;
...
\begin{lstlisting}[style=Default]
function subtract : (lhs : Selection; rhs : Selection) $\to$ $\emptyset$
	// Note: This method should only be invoked on newly-created selections.
	assert empty();
	this := lhs;
	for each n $\in$ rhs.nodes()
...
\begin{lstlisting}[style=Default]
function subtract_using_leaves : (lhs : Selection; rhs : Selection) $\to$ $\emptyset$
	// Note: This method should only be invoked on newly-created selections.
	assert empty();
	for each n $\in$ lhs.nodes()
		layer(0).append(forest().receptive_region_of(n));
